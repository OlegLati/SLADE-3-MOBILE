...
public:
    ...
    // Takes a fresh internal copy of [data, data + size) (via MemChunk::
    // importMem, unchanged from before). Returns false, leaving the
    // session closed, if WadArchive::open() rejects the data -- bad
    // magic, corrupt directory, etc.
    bool open(const unsigned char* data, uint32_t size)
    {
        ...
        if (!wad_->open(*mc_))
        {
            close(); // nothing usable to keep around after a failed open
            return false;
        }
        return true;
    }
    ...
    void close()
    {
        ...
        mc_    = nullptr;
        dirty_ = false;
        pendingSave_.reset(); // Phase 8: stale once the session it was serialized from is gone
    }

    bool        isOpen() const { return wad_ != nullptr && mc_ != nullptr; }
    WadArchive* archive() const { return wad_; }
    MemChunk*   memChunk() const { return mc_; }

    // Modification tracking / dirty state (Phase 6 checklist). Nothing
    // calls markDirty() yet -- there's no edit operation on the native
    // side to call it (that's Phase 7) -- but it's wired up now so the
    // first real edit op has somewhere to report to already, rather than
    // needing another pass through this file to add it later.
    bool isDirty() const { return dirty_; }
    void markDirty() { dirty_ = true; }
    void clearDirty() { dirty_ = false; } // Phase 7: called by saveToFd() on success only

    // Phase 8: Discard Changes. mc_ is the session's own private copy of
    // the bytes the archive was opened from (via importMem() in open()
    // above) -- and per this file's top-of-file "IMPORTANT" comment and
    // saveToFd()'s own priming-loop comment, nothing on the edit side
    // (rename/delete/add/replace/move) ever writes back into mc_ itself;
    // rename/delete/move only touch the WadArchive's in-memory entry
    // list, and add/replace hand new bytes to the *entry* via
    // importMem() on the entry object, not to mc_. That means mc_ is
    // still byte-for-byte the original file, no matter how many edits
    // have happened -- so "undo everything" is just "throw away the
    // WadArchive (and every entry it owns, including any
    // replaced/added entry content) and reparse mc_ from scratch",
    // rather than needing a second pristine copy kept around just for
    // this. Returns false (no-op) if nothing is open.
    bool discardChanges()
    {
        if (!isOpen())
            return false;
        ...
        if (!wad_->open(*mc_))
        {
            // Unreachable in practice -- mc_ parsed successfully once
            // already (that's how the session got opened) and hasn't
            // changed since -- but leave the session closed rather than
            // holding a WadArchive* that failed to open, same as open()
    ...
    // Phase 8: Safe Save's two-step validate-then-commit protocol (see
    // nativeValidateForSave()/nativeCommitSave() below for why this is
    // split into two native calls rather than one -- short version:
    // Kotlin must not open the original document for writing until AFTER
    // validation succeeds, since opening it in "rwt" mode truncates it
    // immediately, before a single byte gets written). pendingSave_ holds
    // the validated, ready-to-write serialization between those two
    // calls. Owned here (not a local in either JNI function) so it
    // survives the gap between them -- two separate JNI calls, two
    // separate stack frames.
    bool hasPendingSave() const { return pendingSave_ != nullptr; }

    void setPendingSave(std::unique_ptr<MemChunk> mc) { pendingSave_ = std::move(mc); }

    MemChunk* pendingSave() const { return pendingSave_.get(); }

    void clearPendingSave() { pendingSave_.reset(); }

private:
    MemChunk*                 mc_          = nullptr;
    WadArchive*               wad_         = nullptr;
    bool                      dirty_       = false;
    std::unique_ptr<MemChunk> pendingSave_ = nullptr;
};

ArchiveSession g_session;
} // namespace
